...
exports.createHr = catchAsync(async (req, res, next) => {
  ...
  const newUser = await prisma.user.create({
    data: {
      email,
      roleId: 1, // Assuming roleId 1 is for HR
      password: hashedPassword,
    },
  });

  // If user is created, create the HR linked to the user
  let hr;
  if (newUser) {
    hr = await prisma.hr.create({
      data: {
        userId: newUser.id,
      },
    });
